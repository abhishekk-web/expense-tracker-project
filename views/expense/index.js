...
function mySave(e){
    ...
    axios.post("http://3.87.2.136:3000/data", object, {headers: {"Authorization": token}})
    .then((response)=>{
  ...
  window.addEventListener("DOMContentLoaded",() => {
    ...
    const data2 = axios.get(`http://3.87.2.136:3000/getData?page=${page}`, {headers: {"Authorization": token}})
  ...
  function deleteUser(userId) {
      ...
      const token = localStorage.getItem('token');
      axios.delete(`http://3.87.2.136:3000/deletedata/${userId}`, {headers: {"Authorization": token}})
      .then((response)=>{
  ...
  function showLeaderboard(){
    ...
    inputElement.onclick = async() => {
      const token = localStorage.getItem('token');
      const userLeaderBoardArray = await axios.get("http://3.87.2.136:3000/premium/showLeaderBoard", {headers: {"Authorization": token}});
  ...
  function editUser(expense, description, category, userId) {
    ...
    document.getElementById('rzp-button1').onclick = async function(e) {
      // const token = localStorage.getItem('token');
      const response = await axios.get('http://3.87.2.136:3000/purchase/premiummembership', {headers: {"Authorization": token}});
      ...
      var options = 
      {
        ...
        "handler": async function(response) {
          const res = await axios.post('http://3.87.2.136:3000/purchase/updatetransactionstatus', {
    ...
    function download(){
      axios.get('http://3.87.2.136:3000/user/download', { headers: {"Authorization" : token} })
      .then((response) => {
